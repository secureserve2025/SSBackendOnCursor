-- Fix Project Delete Function (Final - Fixed Ambiguous Column Reference)
-- This script creates the final working delete_project_cascade RPC function

-- Drop the existing function first to avoid conflicts
DROP FUNCTION IF EXISTS delete_project_cascade(UUID);

-- Create the fixed delete_project_cascade function
...
BEGIN
    ...
    -- Check if project exists and get client_id and status (fixed ambiguous column reference)
    SELECT EXISTS(SELECT 1 FROM projects WHERE id = project_uuid), p.client_id, p.project_status_workflow
    INTO project_exists, project_client_id, project_status
    FROM projects p
    ...
    -- Verify user owns this project by checking client_profiles (fixed column reference)
    IF NOT EXISTS(
        SELECT 1 FROM client_profiles 
        WHERE id = project_client_id AND user_id = current_user_id
    ) THEN
        result := json_build_object(
            'success', false,
