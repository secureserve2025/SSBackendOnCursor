-- Fix Project Delete Function (Updated)
-- This script creates a proper delete_project_cascade RPC function that only deletes from existing tables
...
-- Now create the updated delete_project_cascade function
...
BEGIN
    ...
    BEGIN
        -- Delete project files first (check if table exists)
        SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'project_files' AND table_schema = 'public')
        INTO table_exists;
        ...
        -- Delete work products (check if table exists)
        SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'work_products' AND table_schema = 'public')
        INTO table_exists;
        ...
        -- Delete verification reports (check if table exists)
        SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'verification_reports' AND table_schema = 'public')
        INTO table_exists;
        ...
        -- Delete messages (check if table exists)
        SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'messages' AND table_schema = 'public')
        INTO table_exists;
        ...
        -- Delete project status history (check if table exists)
        SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'project_status_history' AND table_schema = 'public')
        INTO table_exists;
