-- Fix project_files to work like work_products and verification_reports
-- Since RLS is already disabled, we need to fix the storage policies

-- 1. First, let's disable RLS on project_files table (in case it's still enabled)
ALTER TABLE project_files DISABLE ROW LEVEL SECURITY;

-- 2. Drop all existing policies on project_files table
...
-- 3. Disable RLS on storage.objects (this is the key fix)
ALTER TABLE storage.objects DISABLE ROW LEVEL SECURITY;

-- 4. Drop all storage policies that might be blocking uploads
...
-- 5. Verify RLS is disabled on both tables
SELECT 
    schemaname,
    tablename,
    rowsecurity
FROM pg_tables 
WHERE tablename IN ('project_files', 'objects') 
AND schemaname IN ('public', 'storage')
ORDER BY schemaname, tablename;

-- 6. Check if project_files table has any policies left
SELECT 
    policyname,
    cmd
FROM pg_policies 
...
-- 7. Test if we can insert a test record
DO $$
DECLARE
