-- Simple fix for project_files - disable RLS on project_files table only
-- This will allow database inserts while working around storage restrictions

-- 1. Disable RLS on project_files table
...
-- 3. Verify RLS is disabled on project_files table
SELECT 
    schemaname,
    tablename,
    rowsecurity
...
-- 4. Check if project_files table has any policies left
SELECT 
    policyname,
    cmd
FROM pg_policies 
...
-- 5. Test if we can insert a test record
DO $$
DECLARE
